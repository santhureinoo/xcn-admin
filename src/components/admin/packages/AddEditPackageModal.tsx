    ...
    useEffect(() => {
        if (formData.region && formData.gameName && formData.vendor) {
            ...
            if (vendor) {
                ...
                // If editing, restore selected packages based on vendorPackageCodes
                if (pkg && formData.vendorPackageCodes.length > 0) {
        ...
        // Only reset selected packages if not in edit mode or if vendor actually changed
        if (!pkg) {
            setSelectedVendorPackages([]);
            setFormData(prev => ({ ...prev, vendorPackageCodes: [] }));
        }
    }, [formData.region, formData.gameName, formData.vendor, vendors, pkg]);

    // Update form when vendor packages are selected (only for new selections, not when loading edit data)
    useEffect(() => {
